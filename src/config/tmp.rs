...
pub fn read_configs(options: Options) -> Result<ConfigSet, Error>
{
    ...
    };
    {
        ...
        let scan_err = |e| Error::DirScan(e, cfgdir.to_path_buf());
        for entry in try!(Walker::new(cfgdir).map_err(&scan_err)) {
            let epath = try!(entry.map_err(&scan_err)).path();
            ...
            match epath.extension().and_then(|x| x.to_str()) {
                Some("hbs") | Some("handlebars") => {
                    ...
                    try!(File::open(&epath)
                         ...
                         .map_err(|e| Error::ReadTemplate(e,
                            epath.to_path_buf())));
